...
In June 2025, the same prompt-injection vector hit Claude Code, Gemini CLI, and GitHub Copilot simultaneously in what researchers called the "Comment and Control" exploit: a malicious instruction embedded in a public GitHub issue body caused the agents to execute arbitrary shell commands on behalf of the attacker. Days earlier, CVE-2025-54135 (CVSS 9.8) demonstrated a README-based injection that achieved remote code execution via Cursor. Separately, a public audit found 24,000+ secrets exposed in public MCP configuration files — a direct consequence of agents treating all fetched text as trusted context.
